...
export default function BrandsPage() {
    ...
    fetch("/api/brands")
      ...
      .then((list) => {
        if (Array.isArray(list) && list.length) {
          ...
          setItems(defaultBrands.map((name, i) => ({ name, order: i })));
        }
      })
      .catch(() => setItems(defaultBrands.map((name, i) => ({ name, order: i }))))
  ...
  return (
    <div className="p-8 md:p-12 max-w-2xl">
      <h1 className="text-2xl font-bold text-zinc-900 mb-2">Brand Listings</h1>
      <p className="text-zinc-600 text-sm mb-6">Names and order only (Our Core Range section).</p>
